"""CatBoost solo with recall-first top-K threshold."""
...
def main() -> None:
    ...
    meta = {"method": "catboost-recall", "k": k, "threshold_strategy": thresh.strategy, "threshold": thresh.threshold}
    joblib.dump(meta, artifacts_dir / "meta.joblib")
    write_artifacts_manifest(artifacts_dir)

    metrics = {
        "method": "catboost-recall",
        "k": k,
    ...
    save_run_config(run_dir, {"k": k, "threshold": thresh.to_dict()})

    plot_pr_curve(y, oof_proba, plots_dir / "oof_pr_curve.png", "CatBoost OOF PR")
